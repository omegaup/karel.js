(function() {
	...
	var callbacks = {
		...
		invalidCell: function(x, y) {
			self.postMessage(JSON.stringify({ data: { type: 'invalidCell', x: x, y: y } }));
		},
	};
	...
	self.addEventListener("message", function(message) {
		try {
			...
			world.load(worldDoc);
			world.reset();
			self.postMessage(JSON.stringify({ data: { type: 'result', value: validate(world, callbacks) } }));
		} catch(ex) {
			console.error(ex, ex.name);
			self.postMessage(JSON.stringify({ error: ex && (ex.message || ex.name) || '' }));
		}
	}, false);
